...
const getWeeks = (month) => {
    ...
    const lastDayofMonth = adjustDoW(moment(firstDay, 'YYYY-MM-DD').add(daysInMonth - 1, 'days').day());

    const beforeDays = Array(firstDayofMonth).fill(0);
    const actualDays = Array(daysInMonth).fill(0).map((item, index) => {
...
const DateInputCal = ({ month, onSelectDate, active, disabledFn }) => {
    ...
    return (
        <table className={styles.root}>
            <thead>
                <tr>
                    {dayNames.map(name => (<th className={styles.head} key={name}>{name}</th>))}
                </tr>
            </thead>
            <tbody>
                {weeks.map(week => (
