...
const FilterSummary = (props) => {
    const { location, filterMap, resetFilters } = props;

    const parsed = parseSearch(location.search);
    const data = parsed.filter ? Object.entries(parsed.filter).filter(item => Boolean(item[1])) : [];
            ...
            <ul className={styles.list}>
                {data.map(item => (
                    <li className={styles.item} key={item[0]}>
                        <FilterSummaryItem itemKey={item[0]} value={item[1]} filterMapItem={filterMap[item[0]]} />
                    </li>
                ))}
            </ul>
            <span
