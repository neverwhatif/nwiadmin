...
export const formatPoints = (input) =>
    (parseFloat(input || 0, 10) / (config.money ? config.money.factor : 100)).toLocaleString(
        undefined,
        {
...
export const formatDate = (input) => {
    ...
    return moment(input, input.match(/:/) ? 'YYYY-MM-DD HH:mm:ss' : 'YYYY-MM-DD').format(
        'DD/MM/YYYY'
    );
};
